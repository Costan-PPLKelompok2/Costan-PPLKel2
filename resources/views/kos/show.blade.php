...
<div class="container my-5">
      ...
      <div class="card shadow-sm">
        <div class="card-body text-center">
          ...
          @if($kos->status_ketersediaan)
              <a href="{{ route('kos.initiateChat', ['kosId' => $kos->id]) }}" class="btn btn-primary btn-lg mr-2">
                <i class="fa fa-phone mr-2"></i>
                Hubungi Pemilik
              </a>
              <a href="{{ route('review.show', $kos->id) }}" class="btn btn-success btn-lg">
                <i class="fa fa-star mr-2"></i>
                Lihat Review & Rating
              </a>
              @if(Auth::check() && $hasFavorited)
                <form action="{{ route('kos.favorite.destroy', $kos->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
              ...
              @elseif(Auth::check())
                <form action="{{ route('kos.favorite.store', $kos->id) }}" method="POST" style="display:inline;">
                    @csrf
  ...
  @auth
    @if(Auth::id() !== $kos->user_id)
      <a href="{{ route('kos.initiateChat', ['kosId' => $kos->id]) }}"
         ...
         style="position: fixed; z-index: 30; bottom: 24px; right: 100px; border-radius: 50px; padding: 0.75rem 2rem;"
         class="btn btn-primary btn-lg shadow d-flex align-items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" class="me-2" style="width: 1.5em; height: 1.5em;" viewBox="0 0 20 20" fill="currentColor">
  ...
  @else
    <a href="{{ route('login') }}?redirect={{ url()->current() }}"
       ...
       style="position: fixed; z-index: 30; bottom: 24px; right: 100px; border-radius: 50px; padding: 0.75rem 2rem;"
       class="btn btn-secondary btn-lg shadow d-flex align-items-center gap-2">
        <svg xmlns="http://www.w3.org/2000/svg" class="me-2" style="width: 1.5em; height: 1.5em;" viewBox="0 0 20 20" fill="currentColor">
          <path fill-rule="evenodd" d="M18 10c0 3.866-3.582 7-8 7a8.841 8.841 0 01-4.083-.98L2 17l1.338-3.123C2.493 12.767 2 11.434 2 10c0-3.866 3.582-7 8-7s8 3.134 8 7zM7 9H5v2h2V9zm8 0h-2v2h2V9zm-4 0H9v2h2V9z" clip-rule="evenodd" />
        </svg>
        <span>Login</span>
